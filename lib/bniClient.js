...
// BNI API HOSTNAME
const SANDBOX_BASE_URL = 'http://127.0.0.1:1238';
...
class BNIClient {
   ...
   */
  constructor(options = { prod: false, appName, clientId, clientSecret, apiKey, apiSecret, privateKeyPath: '', publicKeyPath: '' }) {
    this.config = options;
  ...
  async getTokenSnapBI() {
    const token = await this.httpClient.tokenRequestSnapBI({
      url: `${this.getBaseUrl()}/snap/v1/access-token/b2b`,
